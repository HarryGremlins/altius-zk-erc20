...
| 证明系统 | **PLONK（Universal SRS）** | 一次性通用 setup（可跨电路复用）；支持 lookup table 实现高效 range check；前向兼容 Phase 2 FHE 桥接证明 |
...
- **BN254** 椭圆曲线运算预编译（EC 点加法、标量乘法、配对）
- PLONK proof 验证预编译（KZG 配对检查）
...
- `v`：余额值
- `r`：随机致盲因子（仅所有者知晓）
- `G`、`H`：**BN254** 椭圆曲线上的两个独立生成点（离散对数关系未知）
...
**证明系统：PLONK（Universal SRS）**

所有证明语句（wrap、保密转账、unwrap）均表达为算术电路，在统一的 PLONK 证明系统下使用通用结构化参考字符串（Universal SRS）进行证明。

- Universal SRS：一次性 setup 仪式，可跨所有电路复用 — 修改或新增电路无需新仪式
- Proof 大小：约 2-4 KB
- 验证：常数时间（EC 多标量乘法）
- Lookup table：无需逐 bit 分解即可实现高效 range check
- 前向兼容：Phase 2 的 FHE 承诺桥接证明可作为新电路加入同一 SRS 下

**为什么不用 Bulletproofs？** Bulletproofs 是专为 Pedersen commitment 上的 range proof 设计的（约 600 字节，无需 trusted setup），但保密转账需要的是*复合*证明（commitment opening + range 约束），超出了 Bulletproofs 原生支持的范围。Phase 1 用 Bulletproofs 意味着 Phase 2 需要从零搭建独立的通用证明管线，导致两套并行的证明系统。PLONK 从一开始就提供统一方案。

**为什么不用 Groth16？** Groth16 产生最小的 proof（约 128 字节）和最便宜的验证（约 200k gas），但需要 per-circuit trusted setup — 每次电路修改都触发新的多方计算仪式。在活跃开发阶段这不现实。电路定稿后，可以选择性地添加 Groth16 wrapper 用于生产优化（这是 zkRollup 的常见模式）。
...
| 有效性证明 | ZK Proof via PLONK（Universal SRS） | PLONK proof + FHE 计算 |
